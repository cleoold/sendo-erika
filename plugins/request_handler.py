...
async def _(session: RequestSession):
    ...
    if sender in SUPERUSERS:
        ...
        await send_to_superusers(
            f'接受了群{session.event["group_id"]}的邀请。(发起人：{sender})')
    else:
...
async def _(session: NoticeSession):
    if session.event['self_id'] != session.event['user_id']:
        return
    await send_to_superusers(
        f'被群{session.event["group_id"]}踢出了。执行者：{session.event["operator_id"]}')
...
async def _(session: NoticeSession):
    if session.event['self_id'] != session.event['user_id']:
        return
    await send_to_superusers(
        f'被群{session.event["group_id"]}设置为管理员。')
...
async def _(session: NoticeSession):
    if session.event['self_id'] != session.event['user_id']:
        return
    await send_to_superusers(
        f'被群{session.event["group_id"]}取消管理员。')
